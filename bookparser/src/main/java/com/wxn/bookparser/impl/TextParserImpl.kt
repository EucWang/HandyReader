...
class TextParserImpl @Inject constructor(
    ...
    override fun needsRescanForMigration(chapters: List<BookChapter>): Boolean =
        txtTextParser.needsRescanForMigration(chapters)

    /**
     * 返回最近一次扫描的 TXT charsetName（仅 TXT 有此概念）。
     *
     * 委托给 [txtTextParser]——非 TXT 格式不会走到本路径（接口默认实现返回 null）。
     *
     * 由 MainReadViewModel 在 replaceChaptersByBookIdUseCase 后调用，回填 BookEntity.txtCharset
     * （详见 docs/plans/plan-txt-unify-byte-offset.md §3.3.3）。
     */
    override fun lastScanCharsetName(bookId: Long): String? =
        txtTextParser.lastScanCharsetName(bookId)
